	...
	for (int y = 0; y < m_rows; y++) {
		for (int x = 0; x < m_cols; x++) {
			if (m_tmaze[x][y] != 'X' && m_tmaze[x][y] != 'S')
				m_openRooms.insert_front(Point(x, y));
			if (m_tmaze[x][y] == 'S') {
				m_tmaze[x][y] = ' ';
				m_start.set(x, y); 
			}
...
void Maze::interact() {
	...
	// Reset Interactions
	for (int i = 0; i < m_nActors; i++)
		m_actors[i]->changeInteract(Interact::ALONE);

	// Permute over Actors
	// If actors are on top of eachother, have them interact
	for (int i = 0; i < m_nActors - 1; i++) {
		Point pos = m_actors[i]->currentRoom();
		for (int j = i + 1; j < m_nActors; j++) { // j never 0
			if (pos == m_actors[j]->currentRoom()) {
				// Actor 0 is nemo, any interaction between nemo and anything
				// else is an ATTACK
				if (i == 0) {
					m_actors[i]->changeInteract(Interact::ATTACK);
					m_actors[j]->changeInteract(Interact::ATTACK);
				}
				else { 
					...
					if (m_actors[i]->getInteract() != Interact::ATTACK) {
						m_actors[i]->changeInteract(Interact::GREET);
						m_actors[j]->changeInteract(Interact::GREET);
					}
				}
			}
		}
...
void Maze::draw() const {
	...
	for (int i = 0; i < m_nActors; i++) {
		...
		char& c = m_renderedMaze[pos.getX()][pos.getY()];
		switch (c) {
	...
	if (!(m_player->stuck() || m_player->foundExit())) {
		...
		if(target.getX()>=0 && target.getY() >= 0)
			m_renderedMaze[target.getX()][target.getY()] = 'T';
	}
...
int Maze::addSharks() {
	...
	do {
		...
		p = m_openRooms.get(index);
	
	} while ((p == m_player->currentRoom())); // Don't start where the player starts
